MSDtrans <- function(Y,X,auxYlist,auxXlist,B,L,eta,
  ...
  # to save validation prediction error
  errlist_screening <- list() # marginal source detection
  for(k1 in 1:nfold_choiceforC){
    ...
    ############## Prediction errors for each of canddiates of C #########################
    pe_byC <- foreach(gg= 1:length(C)) %dopar%{
      library(foreach)
      auxYlist1 <- auxXlist1 <- list()
      auxvec1 <- which( errorvecmean < ((1+C[gg])*mean(lassoerr_vec)) )
      if(length(auxvec1)==0){
        # the case where any source isn't selected
        cv1err <- Lassoerr_tra
      }else{
        # auxvec1: marginally detected sources
        for(g in 1:length(auxvec1)){
          auxYlist1[[g]] <- auxYlist[[ auxvec1[g] ]]
    ...
    errlist_screening[[k1]] <- unlist(pe_byC)
  ...
  err_screening_mean <- apply(do.call('rbind', errlist_screening),2,mean) # vector(the number of compoents = the number of cnadidates of C)
  ...
  opt_C <- C[which.min( unlist(err_screening_mean) )] # optimal C for marginal source detection
  auxvec1_opt <- which( errorvecmean < ((1+opt_C)*mean(lassoerr_vec)) )
  detected_sources_screening <- auxvec1_opt
  if(length(auxvec1_opt)==0){
    cv.nuclear(Y=Y, X=X, B=NULL,
               L=NULL,eta=eta,lamseq = lamseq_w,
               tol=tol,maxiter=maxiter) -> cvLasso
