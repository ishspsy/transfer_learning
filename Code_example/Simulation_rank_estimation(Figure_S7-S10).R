...
sources <-generator_source_rankestimationsimul(rep=100, n=300,p=80,q=20,B=targestset$B,
                                                                              corx=0.5,cory=0,
...
df <- data.frame(
  r = rep(c(3,7), each=8),
  h = rep(c(0,2,10,18), each=2, times=2),
  Method = rep(c("SCAD","NR"), times=8),
  PCR = c( sum(rankvec_transSCAD==8), sum(rankvec_transNR==8 ) )/5,
  r_hat =  c( mean(rankvec_transSCAD), mean(rankvec_transNR)),
  r_sd =  c( sd(rankvec_transSCAD), sd(rankvec_transNR)),
  EE = c(mean(Errorvec_transSCAD), mean(Errorvec_transNR)),
  EE_sd = c(sd(Errorvec_transSCAD), sd(Errorvec_transNR))
)

df$Scenario <- paste0("r=", rep(c(3,7), each=8), ", h=", df$h)

save("df", file="f_S7.RData")
